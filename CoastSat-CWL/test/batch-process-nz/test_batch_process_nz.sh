#!/bin/bash
# Test script for batch-process-nz.cwl tool
...
# Check Docker availability and image
echo "Step 0: Checking Docker and image availability..."
echo "----------------------------------------"
...
if ! docker images coastsat-cwl:latest --format '{{.Repository}}:{{.Tag}}' | grep -q "coastsat-cwl:latest"; then
    echo "❌ Docker image coastsat-cwl:latest not found"
    echo "   Please build the image first"
    exit 1
fi

echo "✅ Docker is available and image coastsat-cwl:latest exists"
echo ""
...
script:
  class: File
  path: ../../tools/batch-process-nz/batch_process_nz_wrapper.py
EOF
...
if [ -f "$EXPECTED" ]; then
    ...
    FALLBACK="$PROJECT_ROOT/../CoastSat-minimal/data/nzd0001/transect_time_series.csv"
    if [ -f "$FALLBACK" ]; then
