#!/usr/bin/env python3
"""
Wrapper script for batch-process-sar CWL tool.
Downloads satellite imagery from Google Earth Engine and extracts shorelines
for a single SAR site. Generates transect time series CSV file.
"""
...
CRS = 3003  # SAR uses EPSG:3003, different from NZ (2193)
...
def process_site(
    ...
    """
    Process a single SAR site: download imagery, extract shorelines, compute transect intersections.
    ...
    # Get reference shoreline for this site (SAR: NOT flipped, unlike NZ)
    ...
    settings["reference_shoreline"] = ref_sl  # SAR: Not flipped
    ...
    # SAR: Do NOT flip shorelines (unlike NZ which flips them)
    # output['shorelines'] = [np.flip(s) for s in output['shorelines']]
    
    # Quality control (SAR uses 15m threshold, NZ uses 10m)
    output = SDS_tools.remove_duplicates(output)
    output = SDS_tools.remove_inaccurate_georef(output, 15)  # SAR: 15m threshold
...
def main():
    parser = argparse.ArgumentParser(description="Process a single SAR site: download imagery and extract shorelines.")
    ...
    parser.add_argument("--start-date", default='1900-01-01', help="Start date (YYYY-MM-DD)")
