...
try:
    import pyfes
    PYFES_AVAILABLE = True
except ImportError:
    PYFES_AVAILABLE = False
    pyfes = None
...
def compute_tide(coords,date_range,time_step,ocean_tide,load_tide):
    ...
    if not PYFES_AVAILABLE:
        raise ImportError("pyfes is required for compute_tide function. Install it with: pip install pyfes")
...
def compute_tide_dates(coords,dates,ocean_tide,load_tide):
    ...
    if not PYFES_AVAILABLE:
        raise ImportError("pyfes is required for compute_tide_dates function. Install it with: pip install pyfes")
    dates_np = np.empty((len(dates),), dtype='datetime64[us]')
    for i,date in enumerate(dates):
        dates_np[i] = datetime(date.year,date.month,date.day,date.hour,date.minute,date.second)
    lons = coords[0]*np.ones(len(dates))
