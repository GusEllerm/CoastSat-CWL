...
# Get project root directory (CoastSat-minimal/)
project_root = Path(__file__).parent.parent
os.chdir(project_root)

# Get repository root directory (parent of CoastSat-minimal/)
repo_root = project_root.parent

# Load environment variables from .env file
# Check both project root and repo root for .env file
env_paths = [project_root / '.env', repo_root / '.env']
for env_path in env_paths:
    if env_path.exists():
        load_dotenv(env_path)
        break
else:
    # If no .env found, try loading from current directory (might be set via environment)
    load_dotenv()

def find_credential_file(filename: str, project_root: Path, repo_root: Path) -> Path:
    """
    Find a credential file, checking project root first, then repo root.
    
    Args:
        filename: Name of the credential file (e.g., '.private-key.json')
        project_root: Project root directory (CoastSat-minimal/)
        repo_root: Repository root directory (parent of CoastSat-minimal/)
    ...
    """
    # Check project root first (for backward compatibility)
    credential_path = project_root / filename
    if credential_path.exists():
        return credential_path
    
    # Check repo root (for shared credentials)
    credential_path = repo_root / filename
    if credential_path.exists():
        return credential_path
    
    # File not found in either location
    raise FileNotFoundError(
        f"{filename} not found in {project_root} or {repo_root}. "
        f"See docs/setup.md for credential setup guidance."
    )
...
# Get private key path from environment or use default
private_key_path_env = os.getenv('GEE_PRIVATE_KEY_PATH')
if private_key_path_env:
    # If explicitly set, use it (could be absolute path)
    private_key_path = Path(private_key_path_env)
    if not private_key_path.is_absolute():
        private_key_path = project_root / private_key_path
else:
    # Otherwise, search for .private-key.json in project root or repo root
    private_key_path = find_credential_file('.private-key.json', project_root, repo_root)
...
transects_gdf = (
    gpd.read_file("inputs/transects_extended.geojson").to_crs(CRS).drop_duplicates(subset="id")
)
...
if TEST_MODE and TEST_SITES:
    ...
    if len(poly) == 0:
        print(f"⚠️  WARNING: TEST_SITES={TEST_SITES} doesn't match any SAR sites!")
        print(f"   Available SAR sites: {original_sites}")
        print(f"   Note: SAR sites must start with 'sar' prefix (e.g., 'sar0001')")
        print(f"   Processing all SAR sites instead...")
        # Reset to all SAR sites if TEST_SITES doesn't match
        poly = gpd.read_file("inputs/polygons.geojson")
        poly = poly[poly.id.str.startswith("sar")]
        poly.set_index("id", inplace=True)
    else:
...
print(f"Processing {len(poly)} SAR site(s): {list(poly.index)}")
