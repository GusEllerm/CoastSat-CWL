...
3. **Configure GDAL if required for your platform.**

4. **Provide credentials:**
   - Google Earth Engine service account and private key (`.private-key.json`).
   - NIWA Tide API key in `.env`.
   
   **Note:** Credentials can be placed in either:
   - Repository root (`CoastSat-CWL/.private-key.json` and `CoastSat-CWL/.env`) - **Recommended** for sharing between `CoastSat-minimal/` and `CoastSat-CWL/`
   - Project root (`CoastSat-minimal/.private-key.json` and `CoastSat-minimal/.env`) - Alternative for project-specific credentials
   
   Scripts automatically search both locations.

**Important:** Always activate the virtual environment before running the workflow:
```bash
source .venv/bin/activate  # On macOS/Linux
